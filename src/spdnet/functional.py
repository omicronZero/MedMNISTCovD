# taken from https://gitlab.lip6.fr/schwander/torchspdnet/-/blob/master/torchspdnet/functional.py?ref_type=heads
...
def init_bimap_parameter(W: StiefelParameter) -> None:
...
def init_bimap_parameter_identity(W: StiefelParameter) -> None:
    """ initializes to identity a (ho,hi,ni,no) 4D-StiefelParameter"""
    ho, hi, ni, no = W.shape

    for i in range(ho):
        for j in range(hi):
            W.data[i, j] = torch.eye(ni, no)
...
def bimap_channels(X: torch.Tensor, W: torch.Tensor) -> torch.Tensor:
    """
...
default_eig_mode: Literal['eig', 'svd'] = 'svd'


def modeig_forward(P: torch.Tensor,
                   op,
                   eig_mode: Literal['eig', 'svd'] = default_eig_mode,
                   param=None):
    """
    ...
    # U, S = torch.zeros_like(P, device=P.device), torch.zeros(batch_size, channels, n, dtype=P.dtype, device=P.device)
    ...
    # return X, U, S, S_fn
    from torchutil.eig import inveig

    if eig_mode == 'eig':
        Sp, Up = torch.linalg.eigh(P)
    elif eig_mode == 'svd':
        Up, Sp, _ = torch.linalg.svd(P, full_matrices=False)
    else:
        raise ValueError(f'Unsupported `eig_mode`: {repr(eig_mode)}.')

    Sp_fn = op.fn(Sp, param)

    Xp = inveig(Sp_fn, Up)

    return Xp, Up, Sp, Sp_fn
...
def cov_pool(f, reg_mode: Literal['mle', 'add_id', 'adjust_eig'] = 'mle'):
    """
...
def cov_pool_mu(f, reg_mode: Literal['mle', 'add_id', 'adjust_eig']):
    """
    Input f: Temporal n-dimensionnal feature map of length T (T=1 for a unitary signal) (batch_size,n,T)
    Output ret: Covariance matrix of size (batch_size,1,n,n)
    """
    ...
    aug1 = torch.cat((X, alpha * mu), 2)
    aug2 = torch.cat((alpha * mu.transpose(1, 2), torch.ones(mu.shape[0], 1, 1, dtype=mu.dtype, device=f.device)), 2)
    X = torch.cat((aug1, aug2), 1)
    if reg_mode == 'mle':
        ret = X
    elif reg_mode == 'add_id':
        ret = add_id(X, 1e-6)
...
def add_id(P, alpha):
    ...
    for i in range(P.shape[0]):
        P[i] = P[i] + alpha * P[i].trace() * torch.eye(P[i].shape[-1], dtype=P.dtype, device=P.device)
    return P
...
def CongrG(P, G, mode: Literal['pos', 'neg']):
    """
...
def BatchDiag(P: torch.Tensor) -> torch.Tensor:
    """
